...
public class WXPayReport {
    ...
    public static class ReportInfo {
        ...
        public String toLineString(String key) {
            ...
            try {
                String sign = WxUtils.HMACSHA256(sb.toString(), key);
                sb.append(sign);
                return sb.toString();
            }
            catch (Exception ex) {
    ...
    private WXPayReport(final WXPayConfig config) {
        ...
        if (config.shouldAutoReport()) {
            WxUtils.getLogger().info("report worker num: {}", config.getReportWorkerNum());
                    ...
                    public void run() {
                        while (true) {
                            ...
                            try {
                                ...
                                WxUtils.getLogger().info("get first report msg: {}", firstMsg);
                                String msg = null;
                                sb.append(firstMsg); //会阻塞至有消息
                                int remainNum = config.getReportBatchSize() - 1;
                                for (int j=0; j<remainNum; ++j) {
                                    WxUtils.getLogger().info("try get remain report msg");
                                    // msg = reportMsgQueue.poll();  // 不阻塞了
                                    msg = reportMsgQueue.take();
                                    WxUtils.getLogger().info("get remain report msg: {}", msg);
                                    if (msg == null) {
                                        break;
                                    }
                                    else {
                                ...
                                WxUtils.getLogger().warn("report fail. reason: {}", ex.getMessage());
                            }
                        }
                    }
                });
    ...
    public void report(String uuid, long elapsedTimeMillis,
        ...
        long currentTimestamp = WxUtils.getCurrentTimestamp();
        ...
        WxUtils.getLogger().info("report {}", data);
        if (data != null) {
